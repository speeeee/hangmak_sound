...
int len(Vec3 a) { return pow(pow(a.x,2)+pow(a.y,2)+pow(a.z,2),0.5); }
float angle(Vec3 a, Vec3 b) { int la = len(a); int lb = len(b);
...
void paint(World *w) { glLoadIdentity(); glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  glColor3f(1,0,0); glRotatef(30,-1,0,0); d_tri(w->t[0]); }
...
int main() {
  ...
  World *w = new World(); w->t.push_back(triangle(0,0,v3(0,1,0)));
  //glRotatef(30,-1,0,0);
  for(bool r = true;r;) {
