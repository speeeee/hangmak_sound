...
Projectile test_collide(World *w, Projectile pp, Entity e, BoundsCompare b) {
  Projectile orig = w->p; Projectile res = pp;
  orig.pos = vsub3(orig.pos,e.pos); res.pos = vsub3(res.pos,e.pos);
  if(e.bf(proj_xz(pp.pos))) {
    for(int i=0;i<e.t.size();i++) { 
      if(in_triangle(proj_xz(res.pos),e.t[i])&&b(orig,res,e.t[i])) {
        res = (e.cf)(w,orig,e.t[i],res); } } res.pos = vadd3(res.pos,e.pos); } return res; }
void entity_collide(World *w, Projectile pp, BoundsCompare b) { Projectile res = pp;
  for(int j=0;j<w->e.size();j++) {
    if(w->e[j].enable) { res = test_collide(w,res,w->e[j],b); } } w->p = res; }
